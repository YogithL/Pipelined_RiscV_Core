    ...
    task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            if(mem_write > 0) begin
                bit[31:0] wr_base = {write_addr[31:2], 2'b00};
                if(mem_write[0]) SIM_RAM[wr_base + 0] = write_data[7:0];
                if(mem_write[1]) SIM_RAM[wr_base + 1] = write_data[15:8];
                if(mem_write[2]) SIM_RAM[wr_base + 2] = write_data[23:16];
                if(mem_write[3]) SIM_RAM[wr_base + 3] = write_data[31:24];
            end
        end
    endtask
