...
// ---------------------------------------------------------
// Instruction Set Enums (Mapped exactly from SV)
// ---------------------------------------------------------
enum opcodes_e {
    OP_Load   = 0x03, 
    OP_Imm    = 0x13, 
...
enum alu_opps_e {
    ALU_ADD  = 0x0, ALU_SLL  = 0x1, ALU_SLT  = 0x2, ALU_SLTU = 0x3,
    ALU_XOR  = 0x4, ALU_SRL  = 0x5, ALU_OR   = 0x6, ALU_AND  = 0x7,
    ALU_SUB  = 0x8, ALU_SRA  = 0xD
};

enum branch_types_e {
    BR_BEQ  = 0x0, BR_BNE  = 0x1, BR_BLT  = 0x4,
    BR_BGE  = 0x5, BR_BLTU = 0x6, BR_BGEU = 0x7
};

enum width_e {
    BYTE_S = 0x0, HALF_S = 0x1, WORD = 0x2, 
    BYTE_U = 0x4, HALF_U = 0x5
};

// ---------------------------------------------------------
// The Golden Model Class
// ---------------------------------------------------------
...
private:
    ...
    // Helper: Bit extraction with sign extension
    int32_t sign_extend(uint32_t val, int bits) {
        int32_t shift = 32 - bits;
        return (int32_t)(val << shift) >> shift; 
    }

    // Helper: ALU Execution
...
public:
    ...
    uint32_t read_reg(uint8_t idx) { return (idx == 0) ? 0 : regs[idx]; }
    void write_reg(uint8_t idx, uint32_t val) { if (idx != 0) regs[idx] = val; }
    uint32_t read_pc() { return current_pc; }

    // Multi-byte memory read based on width_e
    uint32_t read_mem(uint32_t addr, uint8_t width) {
        uint32_t val = 0;
        val |= memory[addr];
        if (width != BYTE_S && width != BYTE_U) val |= (memory[addr + 1] << 8);
        if (width == WORD) {
            val |= (memory[addr + 2] << 16);
            val |= (memory[addr + 3] << 24);
        }
        // Handle sign extension for loads
        if (width == BYTE_S) return sign_extend(val, 8);
        if (width == HALF_S) return sign_extend(val, 16);
        return val;
    }
...
// ---------------------------------------------------------
// DPI-C Wrapper Interface
// ---------------------------------------------------------
static GoldenProcessor* cpu_model = nullptr;

extern "C" {
    void golden_init() {
        if (!cpu_model) cpu_model = new GoldenProcessor();
        else cpu_model->reset();
    }

    void golden_step(int instr) {
        if (cpu_model) cpu_model->step_instruction((uint32_t)instr);
    }
    ...
    // Backdoor Memory Access for UVM initialization/checking
    void golden_write_mem(int addr, int data, int width_enum) {
        if (cpu_model) cpu_model->write_mem((uint32_t)addr, (uint32_t)data, (uint8_t)width_enum);
    }
